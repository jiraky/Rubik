...
public class KociembaLib extends ResolutionStrategy {
     ...
     *             Thrown if the dimension of the cube is not equal to three, if
     *             the cube has not nine faces per color, if there isn't a
     *             single facelet per color.
     */
    public KociembaLib(final RubikCubeModel m) throws IllegalArgumentException {
        super(m);
        if (m.getDimension() != 3) {
     ...
     * 
     * @return <tt>Kociemba two-phase solver</tt>
     */
    @Override
    public final String toString() {
        return "Kociemba two-phase solver";
    }
    ...
    public static String mapOurCubeModelToKociemba(RubikCubeModel cube) {
        ...
        for (RubikCubeSide side = RubikCubeSide.UP;;) {
            for (int i = 0; i < 3; i++)
                for (int j = 0; j < 3; j++)
                    result += mapOurCubeColorToKociemba(cube
                            .getFace(side, i, j));

            if (side == RubikCubeSide.UP)
                side = RubikCubeSide.RIGHT;
            else if (side == RubikCubeSide.RIGHT)
                side = RubikCubeSide.FRONT;
            else if (side == RubikCubeSide.FRONT)
                side = RubikCubeSide.DOWN;
            else if (side == RubikCubeSide.DOWN)
                side = RubikCubeSide.LEFT;
            else if (side == RubikCubeSide.LEFT)
                side = RubikCubeSide.BACK;
            else if (side == RubikCubeSide.BACK)
                break;
        }
        return result;
    }
    ...
    public static RubikCubeModel mapKociembaModelToOurCube(String kociemba_model) {
        ...
        RubikCubeSide[] sides = { RubikCubeSide.UP, RubikCubeSide.RIGHT,
                RubikCubeSide.FRONT, RubikCubeSide.DOWN, RubikCubeSide.LEFT,
                RubikCubeSide.BACK };
        for (int f = 0; f < 6; f++)
            for (int i = 0; i < 3; i++)
                for (int j = 0; j < 3; j++)
                    cube.setFace(
                            ...
                            i,
                            j,
                            mapKociembaColorToOurCube(kociemba_model.charAt(f
                                    * 6 + i * 3 + j)));
    ...
    public static String mapOurCubeColorToKociemba(RubikCubeFaceColor color) {
        ...
        case YELLOW:
            return "D";
        default:
            throw new UnsupportedOperationException("Cannot convert this color");
        }
    }
    ...
    public static RubikCubeFaceColor mapKociembaColorToOurCube(char color) {
        ...
        switch (color) {
        case 'L':
            return RubikCubeFaceColor.GREEN;
        case 'F':
            return RubikCubeFaceColor.RED;
        case 'U':
            return RubikCubeFaceColor.WHITE;
        case 'D':
            return RubikCubeFaceColor.YELLOW;
        case 'R':
            return RubikCubeFaceColor.BLUE;
        case 'B':
            return RubikCubeFaceColor.ORANGE;
        default:
            throw new UnsupportedOperationException("Cannot convert this color");
        }
    }

    public static Facelet mapFaceAndColorWithFacelet(RubikCubeSide side, int x,
            int y) {
        switch (side) {
        case BACK:
            if (x == 0 && y == 0) {
        ...
        default:
            throw new UnsupportedOperationException(
                    "Cannot convert this face: " + side.toString());
        }
    }
}
